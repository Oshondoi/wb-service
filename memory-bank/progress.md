...
## Latest Update (Jan 25, 2026) - ✅ DEBT GROUPS + OPERATION TYPE + UI REFACTOR:
...
**🗂️ FILES MODIFIED**:
- `fin-report.js` — убраны автокомментарии (~2904), добавлен operation_type в запрос, переименование "Операции" → "Записи" (~1817, ~1843), обновлена логика агрегации (~2769-2840), добавлена кнопка удалить (~2875)
- `database.js` — добавлена функция `findOpenDebtGroup()` (~651-700), обновлена `createCashDebt()` (~720), экспортирована новая функция (~1015)
- `supabase-schema.sql` — добавлена колонка `debt_group_id UUID` и `operation_type TEXT`
- `migration-operation-type.sql` — миграция для добавления operation_type
- `migration-debt-groups.sql` — миграция для добавления debt_group_id

**✅ TESTING**: 
- ✅ Новые долги создаются с уникальным debt_group_id
- ✅ Операции с одним контрагентом добавляются в открытую группу
- ✅ Закрытые долги (сумма=0) не принимают новые операции
- ✅ Тип операции отображается в отдельной колонке
- ✅ Комментарий записывается как есть, без префиксов

**📝 ВАЖНО:**
- Требуется миграция БД (выполнить SQL в Supabase):
  ```sql
  ALTER TABLE cash_debts ADD COLUMN operation_type TEXT DEFAULT 'increase';
  ALTER TABLE cash_debts ADD COLUMN debt_group_id UUID DEFAULT gen_random_uuid();
  CREATE INDEX idx_cash_debts_group_id ON cash_debts(debt_group_id);
  ```
- Старые записи будут работать с fallback логикой
- Группировка решает проблему "один контрагент = один долг"
