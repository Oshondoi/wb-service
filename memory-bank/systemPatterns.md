...
- В тулбаре `/shipments-2` вместо «Обновить» используется действие **«Создать источник»**
...
- Создание партии в `/shipments-2` реализовано как step-by-step wizard в модалке (без browser `prompt/confirm`)
- Wizard `/shipments-2` использует source-first flow: `Этап 1/5` источник → `2/5` компания → `3/5` кол-во складов → `4/5` склады → `5/5` товары
- Заголовки шагов стандартизированы в формате `Этап X/5`
- Wizard-паттерн: в каждый момент времени виден только один шаг; footer фиксированный; навигация включает `Back`
- Для полей выбора используются явные пустые placeholder-значения, чтобы предотвратить неявный выбор по умолчанию
- При незавершённом wizard-draft закрытие модалки требует подтверждения (защита от случайной потери данных)
- Доменная модель сохранения: одна партия может порождать несколько поставок (по одной записи поставки на выбранный склад)
- Модалка работы с записью поставки в `/shipments-2` зафиксирована как read-only «Детали поставки» (без inline-редактирования)
- Паттерн открытия деталей: клик по телу строки только в зоне колонок `Дата...Автор`; checkbox и action-иконки исключены из click-target
- Для кликабельной зоны строки обязательны явные affordance: hover-подсветка + `cursor: pointer`
- Шаг товаров в wizard реализует поток: barcode (ручной/камера) → warehouse → box → qty → add item
- Выбор коробов в шаге товаров зависит от текущего выбранного склада (`warehouse -> boxes`)
- Если в шаге товаров введён номер короба, отсутствующий в выбранном складе, короб создаётся автоматически перед добавлением позиции
- Управление источниками и складами реализовано отдельными модалками (создание + список + удаление) в едином стиле проекта
- Горизонтальные ленты выбора складов и коробов должны использовать принудительно видимый horizontal scrollbar (не только `auto`)
- Список добавленных товарных позиций на шаге товаров должен иметь отдельную вертикальную scroll-область
- В блоке списка `/shipments-2` используется паттерн массовых действий: `Выбрано: N` + `Удалить выбранные` + фильтры
- Новые контролы списка (count/filter/delete) размещаются над таблицей в секции списка, а не в верхнем action-toolbar
- Семантика списка зафиксирована: колонка `Партия` отображает источник, колонка `Название поставки` отображает магазин
- Семантика фильтров синхронна колонкам: фильтр `Партия` -> источники, фильтр `Поставка` -> магазины
