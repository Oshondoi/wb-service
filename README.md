...
Полноценный аналитический сервис для сбора максимум данных о товарах Wildberries с многоуровневыми фолбэками, публичными CSV для интеграции с Google Sheets и **модулем финансовых отчетов**.
...
4. **🆕 Вести финансовый отчет по API ключам Wildberries с расчетом чистой прибыли** — автоматический учет продаж, комиссий, логистики и прозрачный расчет прибыли.
5. Стать базой для будущей аналитики: динамика цен, густота остатков, доступность по регионам, качество карточки (изображения / отзывы).
...
| `/fin-report` | HTML (auth) | **🆕 Финансовый отчет**: учет продаж по API, расчет чистой прибыли |
...
## 📈 Модуль финансовых отчетов (В разработке)

**Новый функционал** для комплексного учета финансов продаж на Wildberries:

### Возможности:
- **Интеграция с API Wildberries** — автоматический сбор данных о продажах, заказах, возвратах через API ключи
- **Расчет чистой прибыли** — прозрачный учет всех расходов:
  - Комиссия маркетплейса (WB)
...
- **Детальная аналитика**:
  ...
  - Динамика прибыли (графики, тренды)
  - ABC-анализ товаров
  - Рентабельность по артикулам
- **Экспорт отчетов** — выгрузка в Excel/CSV для дальнейшего анализа
- **Безопасность** — шифрование и защищенное хранение API ключей
...
|--------|----------|--------|
| **Финансовый отчет** | Полноценный модуль учета прибыли по API WB | 🚧 В разработке |
